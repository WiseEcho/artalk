...
func CommentCreate(app *core.App, router fiber.Router) {
	router.Post("/comments", common.LimiterGuard(app, func(c *fiber.Ctx) error {
		...
		if errors.Is(err, common.ErrTokenUserNotFound) {
			vercaniumUser, err := app.Dao().GetVercaniumUserInfo(user.ID)
			if err != nil {
				log.Errorf("[CommentCreate] GetVercaniumUserInfo id:%d,error:%v", user.ID, err)
				return common.RespError(c, 500, i18n.T("Comment failed"))
			}
			user.LastIP = ip
			user.LastUA = ua
			user.Avatar = vercaniumUser.Avatar
			user.Name = vercaniumUser.Alias
			user.Email = vercaniumUser.Email
